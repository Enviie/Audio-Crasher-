...
while _G.Crasher do 
    for i, v in pairs({unpack(BPack:GetChildren()), unpack(Char:GetChildren())}) do
        if v:IsA("Tool") and string.find(v.Name:lower(), "boombox") or string.find(v.Name:lower(), "radio") then
            v.Parent = Char
            v:FindFirstChildWhichIsA("RemoteEvent", true):FireServer("PlaySong", "" .. ConvertID)
        end
    end
    wait()
end   
